...
-- ==============================================================================
-- SECTION 3: PHASE 8 — PRODUCTION DAILY EDGE ENGINE & PUBLICATION
-- ==============================================================================
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Public read daily edge publications' AND tablename = 'daily_edge_publications') THEN
    CREATE POLICY "Public read daily edge publications" ON public.daily_edge_publications FOR SELECT USING (status IN ('published', 'pass_day'));
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Public read daily edge legs' AND tablename = 'daily_edge_legs') THEN
    CREATE POLICY "Public read daily edge legs" ON public.daily_edge_legs FOR SELECT USING (
      EXISTS (
        SELECT 1 FROM public.daily_edge_publications p
        WHERE p.id = daily_edge_legs.publication_id AND p.status = 'published'
      )
    );
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Service role write daily edge publications' AND tablename = 'daily_edge_publications') THEN
    CREATE POLICY "Service role write daily edge publications" ON public.daily_edge_publications FOR ALL USING (auth.role() = 'service_role');
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Service role write daily edge legs' AND tablename = 'daily_edge_legs') THEN
    CREATE POLICY "Service role write daily edge legs" ON public.daily_edge_legs FOR ALL USING (auth.role() = 'service_role');
  END IF;
END $$;
